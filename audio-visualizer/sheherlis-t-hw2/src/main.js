...
const loadXmlXHR = () => {
	...
	xhr.onload = (e) => {
		...
		for (let i = 0; i < songs.length; i++) {
			if (i == 0) {
				document.querySelector("#select-track").innerHTML += `<option value="${songs[i].path}" selected>${songs[i].title}</option>`
			}
			else {
				document.querySelector("#select-track").innerHTML += `<option value="${songs[i].path}">${songs[i].title}</option>`
			}
		}
	};
